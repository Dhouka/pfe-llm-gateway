...
public class GuardrailPolicy {
    ...
    // Multilingual on purpose: this gateway's stated target clients are Tunisian banks
    // (Zitouna Bank, BNA-style institutions per CLAUDE.md), where end users overwhelmingly
    // write in French and (Tunisian/Modern Standard) Arabic, not English. An English-only
    // keyword pass — the state of this list until 2026-06-25 — let "profits garantis" or
    // "ربح مضمون" through untouched even though they're exact translations of phrases
    // already on the English list. The keyword pass is still just a fast first filter (the
    // semantic pass behind it is language-agnostic by construction, since it asks the model
    // to reason about meaning rather than match substrings), but it shouldn't silently only
    // work in one of this project's three relevant languages.
    private static final List<String> TOXIC_KEYWORDS = List.of(
            ...
            // French
            "idiot", "stupide", "imbécile", "ferme la", "tu ne vaux rien",
            "je te hais", "va te faire foutre",
            // Arabic (mild/common moderation terms; transliterated and script forms)
            "غبي", "احمق", "اخرس", "ما تستحى"
    );
    ...
    private static final List<String> ILLEGAL_FINANCE_KEYWORDS = List.of(
            ...
            // French
            "profit garanti", "rendement garanti", "profit garanti à 100%",
            "délit d'initié", "delit d'initie", "pyramide de ponzi",
            "investissement illégal", "investissement illegal",
            "évasion fiscale", "evasion fiscale", "blanchiment d'argent",
            "fraude financière", "fraude financiere", "manipulation boursière",
            "manipulation boursiere",
            // Arabic
            "ربح مضمون", "عائد مضمون", "تهرب ضريبي", "غسيل أموال", "احتيال مالي",
            "استثمار غير قانوني"
    );
    ...
    private static final List<String> INJECTION_PATTERNS = List.of(
            ...
            "new instructions:",
            // French
            "ignore les instructions précédentes", "ignore les instructions precedentes",
            "ignore toutes les instructions", "oublie tes instructions",
            "tu es maintenant", "nouvelles instructions:",
            // Arabic
            "تجاهل التعليمات السابقة", "تجاهل كل التعليمات", "انت الان", "أنت الآن"
    );
