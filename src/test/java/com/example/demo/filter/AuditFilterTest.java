...
class AuditFilterTest {
    ...
    void fallsBackToStatusCodeWhenNoAttributesSet() throws InterruptedException {
        ...
        // Was "/audit" — now excluded from audit logging (see AuditFilter's 2026-06-30 fix
        // for the dashboard self-polling/lock-convoy issue), so this test's path is changed
        // to a generic, non-excluded one. "/audit" here was always just an arbitrary stand-in
        // path to exercise the filter's own logic, never a check that /audit specifically
        // gets logged.
        ServerWebExchange exchange = MockServerWebExchange.from(MockServerHttpRequest.get("/some-protected-resource").build());
    ...
    void allowedRequestIsLoggedAsAllowed() throws InterruptedException {
        AuditLogStore store = newStore();
        AuditFilter filter = new AuditFilter(store);
        // Was "/audit" — see comment in fallsBackToStatusCodeWhenNoAttributesSet above.
        ServerWebExchange exchange = MockServerWebExchange.from(MockServerHttpRequest.get("/some-protected-resource").build());
    ...
    @Test
    void auditAndActuatorPathsAreNotLogged() throws InterruptedException {
        // Locks in the 2026-06-30 fix: the dashboard polls /audit and /audit/summary every
        // ~3s, and each poll used to write its own audit entry — a self-amplifying feedback
        // loop that also queued up behind AuditLogStore.persist()'s synchronized SQLite write,
        // producing observed latencies of 5-190+ seconds on requests doing no real work.
        AuditLogStore store = newStore();
        AuditFilter filter = new AuditFilter(store);

        for (String path : new String[]{"/audit", "/audit/summary", "/audit/detail/abc-123", "/actuator/health"}) {
        ...
        // Give any (incorrectly fired) async logging a moment to land before asserting absence.
        Thread.sleep(50);
        assertEquals(0, store.getLogs().size());
    }
