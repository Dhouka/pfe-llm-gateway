...
class PiiFilterTest {
    ...
    void setUp() {
        ...
        // Pass-through: simulate pii-service finding nothing extra and succeeding.
        when(piiServiceClient.anonymize(any()))
                .thenAnswer(inv -> Mono.just(new PiiServiceClient.Result(inv.getArgument(0), 0, false)));
        filter = new PiiFilter(piiServiceClient, true);
    }
    ...
    void piiCountIncludesPresidioSecondPassEntities() {
        when(piiServiceClient.anonymize(any()))
                .thenAnswer(inv -> Mono.just(new PiiServiceClient.Result(inv.getArgument(0), 2, false)));
    ...
    @Test
    void piiCountReflectsActualOccurrenceCountNotJustDistinctPatternTypes() {
        // Until 2026-06-25, matchCount counted "did this pattern type fire at least
        // once" (capped at 4), not real occurrences — this body has 3 emails + 1 phone
        // number, which the old code would have reported as piiCount=2 (one per pattern
        // type) instead of the true count of 4.
        ServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.post("/llm/chat")
                        .body("{\"message\":\"Contact a@gmail.com, b@gmail.com or " +
                                "c@gmail.com, or call 23456789\"}"));

        filter.filter(exchange, ex -> Mono.empty()).block();

        assertEquals(true, exchange.getAttribute(AuditFilter.ATTR_PII_DETECTED));
        assertEquals(4, (int) exchange.getAttribute(AuditFilter.ATTR_PII_COUNT));
    }

    @Test
    void blocksRequestWithHttp503WhenPiiServiceFailsAndFailClosedEnabled() {
        when(piiServiceClient.anonymize(any()))
                .thenAnswer(inv -> Mono.just(new PiiServiceClient.Result(inv.getArgument(0), 0, true)));
        ...
        assertFalse(chainCalled.get(), "request must not be forwarded to the chain/LLM");
        assertEquals(503, exchange.getResponse().getStatusCode().value());
        assertEquals("BLOCKED_PII_SERVICE_DOWN", exchange.getAttribute(AuditFilter.ATTR_EVENT));
    }

    @Test
    void forwardsRequestWhenPiiServiceFailsButFailClosedDisabled() {
        PiiFilter failOpenFilter = new PiiFilter(piiServiceClient, false);
        when(piiServiceClient.anonymize(any()))
                .thenAnswer(inv -> Mono.just(new PiiServiceClient.Result(inv.getArgument(0), 0, true)));
        ...
        AtomicReference<Boolean> chainCalled = new AtomicReference<>(false);
        failOpenFilter.filter(exchange, ex -> {
            chainCalled.set(true);
            return Mono.empty();
        }).block();

        assertTrue(chainCalled.get(), "fail-open mode must still forward the request");
    }
}
